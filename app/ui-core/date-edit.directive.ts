...
export class DateEdit implements OnInit {
	...
	@HostListener('keydown') onKeyDown($event: KeyboardEvent) {
		...
		console.log('keydown', keyboardEvent.keyCode);

		if (this.nokey) return;

		if (keyboardEvent.metaKey && keyboardEvent.keyCode == 90) {
			let history = this.control['_history'];
			if (history && history.length > 1)
				this.control.setValue(history[history.length - 2]);
			return;
		}
